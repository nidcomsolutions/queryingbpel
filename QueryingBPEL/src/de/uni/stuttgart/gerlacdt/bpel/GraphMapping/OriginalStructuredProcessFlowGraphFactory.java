...
/**
 * The StructuredProcessFlowGraphFactory class is a creator for ProcessFlowGraph
...
public class OriginalStructuredProcessFlowGraphFactory extends OriginalProcessFlowGraphFactory {
	/**
	 ...
	 *            be generated.
	 */
	public OriginalStructuredProcessFlowGraphFactory(Process process) {
		super(process);
	}
	...
	private ActivityNode createStructureHelperNode(
		...
		// Give different name for start and end nodes
		if (isStart) {
		...
		// create ActivityNode with the above stats
		// for structuralHelper, no not-null test is necessary because
		// eContainer always exists.
		ActivityNode node = new StructuredActivityNode(IdentifierCacheHandler
				.getID(structureHelper).toString(), structureHelperName,
				structureHelperType, IdentifierCacheHandler.getID(
						structureHelper.eContainer()).toString(), isStart);
