...
public abstract class OriginalProcessFlowGraphFactory {
	...
	/**
	 * holds the first added node of the graph
	 */
	protected Activity startActivity = null;
	 ...
	 *            be generated.
	 */
	public OriginalProcessFlowGraphFactory(Process process) {
	...
	protected void handleBasicActivity(Activity activity,
		...
		processFlowGraph.getProcessGraph().addVertex(activityNode);
	...
	protected void addStructuredStartEndNodeToProcessGraph(
		...
		processFlowGraph.getProcessGraph().addVertex(startNode);

		// add endNode to processGraph
		processFlowGraph.getProcessGraph().addVertex(endNode);
	}
	...
	protected void handleStructuredActivityInSpecialContainer(
		...
		if (ContainerActivityStart != null
			...
			processFlowGraph.getProcessGraph().addEdge(ContainerActivityStart,
					startEndNodes[0]);
			...
			// activity node
			if (targets == null) {
				processFlowGraph.getProcessGraph().addEdge(
						ContainerActivityStart, startEndNodes[0]);
			}
			// set lastAddedNode to the new node
			lastAddedNode = startEndNodes[0];
	...
	protected void handleBasicActivityInSpecialContainer(Activity activity,
		...
		if (containerActivityStart != null
			...
			if (targets == null) {
				processFlowGraph.getProcessGraph().addEdge(
						containerActivityStart, activityNode);
			}
			// set lastAddedNode to the new node
			lastAddedNode = activityNode;
				...
				if (!flowLinkMap.containsKey(linkName)) {
					...
					if (flowLinkHelperObj instanceof FlowLinkHelperBasicActivity) {
						...
						ActivityNode sourceNode = flowLinkHelperBasicActivity
								.getBasicActivityNode();
						processFlowGraph.getProcessGraph().addEdge(sourceNode,
								activityNode);
					} else if (flowLinkHelperObj instanceof FlowLinkHelperStructuredActivity) {
						FlowLinkHelperStructuredActivity flowLinkHelperStructuredActivity = (FlowLinkHelperStructuredActivity) flowLinkHelperObj;
	...
	/**
	 * Adds all corresponding links to the ProcessFlowGraph for the given
			...
			for (Source source : sourcesList) {
				...
				if (!flowLinkMap.containsKey(linkName)) {
					...
					if (flowLinkHelperObj instanceof FlowLinkHelperBasicActivity) {
						...
						ActivityNode targetNode = flowLinkHelperBasicActivity
								.getBasicActivityNode();
						processFlowGraph.getProcessGraph().addEdge(endNode,
								targetNode);
					} else if (flowLinkHelperObj instanceof FlowLinkHelperStructuredActivity) {
						FlowLinkHelperStructuredActivity flowLinkHelperStructuredActivity = (FlowLinkHelperStructuredActivity) flowLinkHelperObj;
						ActivityNode targetNode = flowLinkHelperStructuredActivity
	...
	protected void setStartActivity() {
		if (startActivity instanceof Invoke || startActivity instanceof Receive
				|| startActivity instanceof Reply || startActivity instanceof Assign
				|| startActivity instanceof Throw || startActivity instanceof Wait
				|| startActivity instanceof Empty || startActivity instanceof Rethrow
				|| startActivity instanceof Exit
				|| startActivity instanceof ExtensionActivity) {
			processFlowGraph.setStartActivity(DbGraphPropertiesFactory.getInstance().createActivityNode(startActivity));
		} else if (startActivity instanceof Sequence || startActivity instanceof If || startActivity instanceof While
		|| startActivity instanceof RepeatUntil || startActivity instanceof Flow 
		|| startActivity instanceof Pick || startActivity instanceof ForEach 
		|| startActivity instanceof Scope){
			processFlowGraph.setStartActivity(DbGraphPropertiesFactory.getInstance().createStructuredActivityNode(startActivity, true));
		} else {
