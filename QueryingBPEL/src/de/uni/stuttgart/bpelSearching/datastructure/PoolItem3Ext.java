...
public class PoolItem3Ext {
	 ...
	 */
	public PoolItem3Ext(ActivityNode querynode, NodeRegionEncoding processnode,
		...
		if (childrenReferencesMap == null) {
			this.childrenReferencesMap = new HashMap<ActivityNode, PoolItem3ExtChildReferences>();
		} else {
	...
	public String toString() {
		String tempStr = "";
		if ((querynode != null) && (processnode != null)) {
			tempStr += ("Q: " + querynode.getActivityID() + " D: " + processnode.getActivityID());
			...
			if (children.isEmpty()) {
				...
				for (PoolItem3Ext child : children) {
					tempStr += ("CQ: " + child.getQuerynode().getActivityID() + " CD: " +
							child.getProcessnode().getActivityID() + " || ");
				}		
			}
		}
		return tempStr;
    ...
    /**
     * Gets all referenced elements in the child pool associated with <code>childQ</code>.
     * 
     * @param childQ a child node of <code>querynode</code>
     * 
     * @return the current child pool element associated with <code>childQ</code>.
     */
	public Set<PoolItem3Ext> getChildren(ActivityNode childQ) {
		Set<PoolItem3Ext> result = new HashSet<PoolItem3Ext>();
		if (childrenReferencesMap.get(childQ) != null) {
			result.addAll(childrenReferencesMap.get(childQ).getChildReferences());
		}
		return result;
	}
